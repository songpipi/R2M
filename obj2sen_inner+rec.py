...
tf.flags.DEFINE_string('job_dir', 'rebuttal/2step/img', 'job dir')
...
tf.flags.DEFINE_integer('max_steps', 100, 'training steps')
...
tf.flags.DEFINE_string('obj2sen_ckpt', 'rebuttal/2step/text/model.ckpt-5700', 'initialization checkpoint')
...
def model_fn(features, labels, mode, params):
  ...
  train_var = []
  # train_var.extend(tf.trainable_variables('fully_connected'))
  ...
  if not FLAGS.multi_gpu or opt._graph_state().is_the_last_tower:
    ...
    train_hooks = [tf.train.LoggingTensorHook(
      {'noise': noise, 'pred': pred, 'loss_m':loss_inner,'loss_r':loss_rec}, every_n_iter=100)]
  ...
  def init_fn(scaffold, session):
    ...
    if FLAGS.obj2sen_ckpt:
      ...
      # full_saver.restore(session, FLAGS.obj2sen_ckpt)
...
def input_fn(batch_size):
  ...
  return {'im':im, 'key': key, 'len': lk},{'la':lk}
