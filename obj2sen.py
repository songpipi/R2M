...
tf.flags.DEFINE_string('job_dir', 'rebuttal/h1/xe', 'job dir')
...
tf.flags.DEFINE_integer('pad_length', 30, 'pad len')

tf.flags.DEFINE_integer('max_steps', 5000, 'training steps')
...
def input_fn(batch_size, subset='train'):
  ...
  key, lk, sentence_pad, ls = iterator.get_next()
  return {'key': key, 'len': lk}, {'sentence': sentence_pad, 'len': ls}


def main(_):
  os.environ['TF_ENABLE_WINOGRAD_NONFUSED'] = '3'
