...
sys.path.insert(0, TF_MODELS_PATH + '/research/im2txt/im2txt/inference_utils')
sys.path.append(TF_MODELS_PATH + '/research')
sys.path.append(TF_MODELS_PATH + '/research/object_detection')
from build_mscoco_data import _create_vocab
import vocabulary
from utils import label_map_util

tf.enable_eager_execution()

flags.DEFINE_bool('new_dict', True, 'generate a new dict')
...
def get_open_image_categories():
  ...
  category_name.sort()
  plural_file = '../data/plural_words_v4.json'
...
def sentence_generator():
  ...
  with open('../data/GCC/sentences_gcc.pkl', 'r') as f:
    captions = pkl.load(f)
  ...
  if FLAGS.new_dict:
    ...
    word_counts = [i for i in vocab if i[0] in category_set or i[1] >= 15]
  ...
  with open('../data/GCC/all_ids_gcc.pkl', 'w') as f:
    pkl.dump(all_ids, f)
...
def main(_):
  ...
  tfrec = tf.data.experimental.TFRecordWriter('../data/GCC/sentence_gcc.tfrec')
  tfrec.write(ds)
